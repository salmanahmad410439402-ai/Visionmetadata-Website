 ...
 * @param {string} [props.badge] - Optional badge text (displays in indigo accent)
...
export const SectionHeader = ({
  ...
  return (
    <div className={`${containerClass} ${className}`}>
      {badge && (
        <div className="reveal inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-semibold uppercase tracking-widest mb-4 accent-indigo-light accent-indigo-border text-accent">
          {badge}
        </div>
      )}
 ...
 * Features indigo accent styling with subtle background.
 *
 * @component
 *
 * @example
...
export const SectionLabel = ({ text }: SectionLabelProps) => (
  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-semibold uppercase tracking-widest accent-indigo-light accent-indigo-border text-accent">
    {text}
  </div>
);
