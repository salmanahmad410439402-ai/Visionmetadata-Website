      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {testimonials.map(({ quote, name, role, initial }, i) => (
            <div key={name}
              className={`reveal reveal-delay-${i + 1} card-lift glass-panel rounded-2xl p-7 flex flex-col gap-5 relative group hover:-translate-y-1 hover:border-primary hover:shadow-[0_0_35px_hsl(var(--primary)/0.28)] transition-all duration-300`}>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-muted">
                <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-primary-foreground flex-shrink-0 accent-indigo">
                  {initial}
                </div>
                <div>
